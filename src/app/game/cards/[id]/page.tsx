...
export default async function GameCardDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: card, error: cardError } = await supabase
    .from("game_cards")
    .select(
      "id, rarity, illustration_url, trivia_kids_text, trivia_adult_text, nutrition_summary, ingredients_master(id, name, reading, category, season_months)"
    )
    .eq("id", id)
    .maybeSingle<CardRow>();
  ...
  const badge = RARITY_BADGE_STYLES[card.rarity];
      ...
      <div className="mt-4 rounded-xl border-2 border-white bg-white p-5 shadow-raised">
        ...
        <div className="mt-3 flex items-center gap-2">
          ...
          <h1 className="text-xl font-bold">
            {isOwned ? (
              <FuriganaName name={card.ingredients_master?.name ?? ""} reading={card.ingredients_master?.reading ?? null} />
            ) : (
              "？？？(未入手)"
            )}
          </h1>
        ...
        <div className="mt-2 flex items-center gap-2">
          <span className={`inline-block rounded-full px-2 py-0.5 text-xs ${badge.bg} ${badge.text}`}>
            {RARITY_LABELS[card.rarity]}
          </span>
          {isOwned ? <span className={`text-sm tracking-wide ${badge.text}`}>{rarityStars(card.rarity)}</span> : null}
        </div>
        ...
        {isOwned ? (
          <div className="mt-4 space-y-3 text-sm">
            ...
            {card.trivia_kids_text ? (
              <FuriganaText text={card.trivia_kids_text} className="block rounded-md bg-brand-50 px-3 py-2 text-brand-800" />
            ) : null}
            {card.trivia_adult_text ? (
              <div>
                <p className="text-xs font-bold text-gray-500">もっと詳しく</p>
                <FuriganaText text={card.trivia_adult_text} className="mt-1 block text-gray-700" />
              </div>
            ) : null}
            {card.nutrition_summary ? (
              <div>
                <p className="text-xs font-bold text-gray-500">栄養素</p>
                <FuriganaText text={card.nutrition_summary} className="mt-1 block text-gray-700" />
              </div>
            ) : null}
            {acquiredByName ? (
